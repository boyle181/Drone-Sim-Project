...
double WalletDecorator::getCostForTrip(IEntity* entity) {
  ...
  return (dist / speed) *
         TRIP_MULTIPLIER;  // total time for trip * Cost per unit of time
}

void WalletDecorator::Update(double dt, std::vector<IEntity*> scheduler) {
  /**
   * Robots should only be charged moeny when its picked up. They pay
   * incrementally but the robot makes sure it has enough money for the whole
   * trip
   */
  ...
  if (type.compare("robot") == 0) {
    ...
    } else if (clientValid && GetPickedUp() &&
               GetPosition().Distance(GetDestination()) < 4.0) {
      clientValid = false;
    }
  }
  ...
  if (type.compare("drone") == 0) {
    ...
    if (GetChargingStatus()) {
      ...
      // If either are avaible then the trip should be canceled
               ...
               (GetAvailability() || GetEntity()->GetAvailability())) {
      ...
      // Determines when client is dropped off (nearest entity null in
      // drone:update when dropped off)
